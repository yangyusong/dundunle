...
(function() {
  ...
  */
  ...
  isEnd = function(socket, tabId) {
    var dataout, enimySock, enimyid, host, idName, tab, time;
    tab = c.tabList[tabId];
    if (tab.handler) {
      clearTimeout(tab.handler);
    }
    time = pubApi.getTime();
    if (time - tab.lastTime > c.timers.standSquatTime) {
      host = tab.host;
      ...
      sendApi.sendresult([socket, enimySock], dataout, false);
      kick(tabId, host);
      ...
      return c.tabList[tabId].handler = setTimeout(isEnd, c.timers.checkTime, socket, tabId);
    }
  };
